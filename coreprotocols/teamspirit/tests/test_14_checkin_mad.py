# ./manage.py test --pattern="test_14_checkin_mad.py"
from django.contrib.auth.models import User
from django.test import TestCase
from teamspirit.models import Team, Session, SessionState

class Test14CheckinSession(TestCase):
    ...
    def test_checkin_mad(self):
        ss = SessionState()
        session = Session.objects.get(name="StandUp Test")
        ss.user = self.joan
        ss.session = session
        ss.comments = "I am mad!!"
        ss.state = "Check In - Mad"
        ss.save()
        self.assertEquals(ss.comments, "I am mad!!")
        self.assertEquals(ss.state, "Check In - Mad")
