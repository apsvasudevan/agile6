# ./manage.py test --pattern="test_15_checkin_afraid.py"
from django.contrib.auth.models import User
from django.test import TestCase
from teamspirit.models import Team, Session, SessionState

class Test15CheckinSession(TestCase):
    ...
    def test_checkin_afraid(self):
        ss = SessionState()
        session = Session.objects.get(name="StandUp Test")
        ss.user = self.joan
        ss.session = session
        ss.comments = "I am afraid!!"
        ss.state = "Check In - Afraid"
        ss.save()
        self.assertEquals(ss.comments, "I am afraid!!")
        self.assertEquals(ss.state, "Check In - Afraid")
