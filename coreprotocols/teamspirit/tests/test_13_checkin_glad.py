# ./manage.py test --pattern="test_13_checkin_glad.py"
from django.contrib.auth.models import User
from django.test import TestCase
from teamspirit.models import Team, Session, SessionState

class Test13CheckinSession(TestCase):
    ...
    def test_checkin_glad(self):
        ss = SessionState()
        session = Session.objects.get(name="StandUp Test")
        ss.user = self.joan
        ss.session = session
        ss.comments = "I am glad!!"
        ss.state = "Check In - Glad"
        ss.save()
        self.assertEquals(ss.comments, "I am glad!!")
        self.assertEquals(ss.state, "Check In - Glad")
